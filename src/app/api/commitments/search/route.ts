 ...
 *
 * Invariant I3: requests with pageSize > MAX_PAGE_SIZE return 400.
 */
...
const CommitmentSearchQuerySchema = z.object({
  ...
  /** Bypass cache and fetch a fresh chain snapshot. */
...
function buildSearchCacheKey(
  ...
  const orderedFilters = Object.keys(filters)
    ...
    .reduce<Record<string, string | number | undefined>>((acc, key) => {
      ...
      acc[key] = key === 'asset' && typeof value === 'string' ? value.trim().toUpperCase() : value;
      return acc;
    }, {});
  const payload = JSON.stringify({ ownerAddress: normalizedOwner, ...orderedFilters });
...
function normalizeSearchItem(raw: any): CommitmentSearchItem | null {
  ...
  const status = raw.status as ChainCommitmentStatus;
  ...
  if (
    ...
    !COMMITMENT_STATUS_VALUES.includes(status)
  ) {
    return null;
  }
